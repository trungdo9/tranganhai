...
2. **Pick the mode:**
   ...
   - **`campaign`** — the closed loop: load `.agents/workflows/seo-workflow.md` and drive its 7 phases (baseline audit + metrics → plan [hard stop] → write → publish → distribute → measure → scale/refresh/kill loop). Triggered by `/mk:seo campaign`.
