# Antigravity Notification Hooks

This directory contains notification hooks for Antigravity sessions. These hooks send real-time notifications to Discord and Telegram when Antigravity completes tasks.

## Overview

Antigravity hooks automate notifications and actions at specific points in your development workflow. This project includes notification systems for Discord and Telegram:
...
### Current Setup
The live configuration is `.agents/settings.json` — read it there rather than from a summary that can drift.

### Scout Block Hook (Cross-Platform)
Automatically blocks Antigravity from accessing heavy directories to improve performance.

**Configuration:** Already enabled in `.agents/settings.json`
...
**Behavior:**
- Triggers on Write/replace_file_content tool usage
- Analyzes file line count (LOC)
- Suggests modularization if >200 LOC
- **Non-blocking**: Antigravity continues main task automatically
...
echo '{"tool_input":{"file_path":"/tmp/test-large.js"}}' | node .agents/hooks/modularization-hook.cjs

# Test with small file (should be silent)
echo "console.log('test');" > /tmp/test-small.js
echo '{"tool_input":{"file_path":"/tmp/test-small.js"}}' | node .agents/hooks/modularization-hook.cjs
```
...
**How It Works:**
1. Hook receives file path from Write/replace_file_content tool
2. Counts lines in modified file
3. If >200 LOC, injects suggestion into Antigravity's context
4. Antigravity receives guidance but continues without blocking
5. Exit code 0 ensures non-blocking behavior

### Discord Hook (Automated)
Automatic notifications on Antigravity session events with rich embeds.

**Setup:** [discord-hook-setup.md](./discord-hook-setup.md)

**Quick Test:**
```bash
echo '{"hookType":"Stop","projectDir":"'$(pwd)'","sessionId":"test","toolsUsed":[{"tool":"Read","parameters":{"file_path":"test.ts"}}]}' | ./.agents/hooks/discord_notify.sh
```
...
**Quick Test:**
```bash
./.agents/hooks/send-discord.sh 'Test notification'
```

### Telegram Hook
Automatic notifications on Antigravity session events.

**Setup:** [telegram-hook-setup.md](./telegram-hook-setup.md)

**Quick Test:**
```bash
echo '{"hookType":"Stop","projectDir":"'$(pwd)'","sessionId":"test","toolsUsed":[]}' | ./.agents/hooks/telegram_notify.sh
```
...
### discord_notify.sh
Automated Discord notification hook for Antigravity events with rich embeds.
...
**Usage:**
```bash
./.agents/hooks/send-discord.sh 'Your message here'
```

**Required:** `DISCORD_WEBHOOK_URL` environment variable

### telegram_notify.sh
Automated Telegram notification hook for Antigravity events.
...
2. **.agents/.env** - Project-level Antigravity configuration
3. **.agents/hooks/.env** - Hook-specific configuration
...
**OR `.agents/.env`** (for project-specific overrides):
```bash
# Same variables as above
```

**OR `.agents/hooks/.env`** (for hook-only configuration):
```bash
# Same variables as above
```
...
Hooks are configured in `.agents/settings.local.json`:
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "${CLAUDE_PROJECT_DIR}/.agents/hooks/telegram_notify.sh"
      }]
    }],
    "SubagentStop": [{
      "hooks": [{
...
- Verify `.agents/settings.json` uses `node .agents/hooks/scout-block.cjs`
- Test manually: `echo '{"tool_input":{"command":"ls node_modules"}}' | node .agents/hooks/scout-block.cjs`
...
- Review [Antigravity Documentation](https://antigravity.google/docs)
- Report issues at [Antigravity Support](https://antigravity.google/support)
