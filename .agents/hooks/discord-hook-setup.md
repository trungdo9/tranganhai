...
The Discord hook (`send-discord.sh`) sends rich embedded messages to a Discord channel when Antigravity completes implementation tasks. Messages include session time, project info, and task summaries.
...
3. Click **"New Webhook"**
4. Configure webhook:
   - **Name:** `Antigravity Bot` (or your preference)
...
**Option B: `.agents/.env`** (project-level override):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```

**Option C: `.agents/hooks/.env`** (hook-specific):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```
...
```bash
chmod +x .agents/hooks/send-discord.sh
```
...
```bash
./.agents/hooks/send-discord.sh 'Test notification from Antigravity'
```
...
```bash
./.agents/hooks/send-discord.sh 'Task completed: Added user authentication feature'
```

**With multi-line messages:**
```bash
./.agents/hooks/send-discord.sh 'Implementation Complete
...
```bash
./.agents/hooks/send-discord.sh 'Fixed bug in user'\''s profile page'
```

### Automated Usage (Antigravity Workflow)

Antigravity automatically calls this script when completing implementations. This is configured in `.agents/workflows/development-rules.md`:

```markdown
- When you finish the implementation, send a full summary report to Discord channel
  with `./.agents/hooks/send-discord.sh 'Your message here'` script
```
...
if npm run build && npm run test; then
    ./.agents/hooks/send-discord.sh '✅ Build and tests passed - ready to deploy'
else
    ./.agents/hooks/send-discord.sh '❌ Build or tests failed - deployment blocked'
fi
```
...
{
  "scripts": {
    "deploy": "npm run build && npm run test && ./.agents/hooks/send-discord.sh '✅ Deployment successful'",
    "notify": "./.agents/hooks/send-discord.sh"
  }
}
```
...
**Embed Components:**
- **Title:** 🤖 Antigravity Session Complete
...
```bash
./.agents/hooks/send-discord.sh 'Message with "double quotes" works fine'
```

**Escape single quotes inside single-quoted strings:**
```bash
./.agents/hooks/send-discord.sh 'User'\''s profile updated'
```

**Use double quotes and escape:**
```bash
./.agents/hooks/send-discord.sh "Message with \"escaped quotes\""
```

**For complex messages, use heredoc:**
```bash
./.agents/hooks/send-discord.sh "$(cat <<'EOF'
Multi-line message
...
./.agents/hooks/send-discord.sh "$1"
...
if npm run test; then
    if [[ "$ENVIRONMENT" == "production" ]]; then
        ./.agents/hooks/send-discord.sh '✅ Production deployment successful'
    fi
else
    # Always notify on failures
    ./.agents/hooks/send-discord.sh '❌ Tests failed - deployment aborted'
fi
```
...
**Script Location:** `.agents/hooks/send-discord.sh`
