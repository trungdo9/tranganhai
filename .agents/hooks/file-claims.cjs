 ...
 * mutation to <worktree-root>/.agents/.ck-file-claims.jsonl:
 ...
 *     compaction) via `node .agents/hooks/file-claims.cjs list`.
 *
 * Design constraints:
 ...
 * `session_id`; the run_command tool env exposes CLAUDE_CODE_SESSION_ID. Fallback
...
function claimsPath(root) {
  return path.join(root, '.agents', '.ck-file-claims.jsonl');
}
...
function hookMode() {
  try {
    ...
    const cwd = (payload.workspacePaths && payload.workspacePaths[0]) || payload.cwd || process.cwd();
    const root = worktreeRoot(cwd);
    if (!root) {
      console.log('{}');
      process.exit(0);
    }
    const rel = path.relative(root, path.resolve(cwd, filePath));
    if (rel.startsWith('..')) {
      console.log('{}');
      process.exit(0);
    }
    appendClaim(root, {
      ...
      ts: Date.now(),
      tool: (payload.toolCall && payload.toolCall.name) || payload.tool_name || 'unknown',
    });
    console.log('{}');
    process.exit(0);
  } catch {
