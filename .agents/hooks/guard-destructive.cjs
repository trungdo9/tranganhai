...
function main() {
  ...
  // Support both Antigravity (toolCall.args.CommandLine) and Claude (tool_input.command)
  let command = '';
  ...
  if (process.env.CK_ALLOW_DESTRUCTIVE === '1' || process.env.AGY_ALLOW_DESTRUCTIVE === '1') {
    console.log(JSON.stringify({ decision: 'allow' }));
    process.exit(0);
  }

  const cwd = (payload.toolCall && payload.toolCall.args && payload.toolCall.args.Cwd) || payload.cwd || process.cwd();
  const session = payload.conversationId || payload.session_id || payload.sessionId || process.env.CLAUDE_CODE_SESSION_ID || `ppid:${process.ppid}`;

  const a = tierA(command, cwd);
  if (a) {
    const reason = `BLOCKED (irreversible): ${a}\nOverride for a deliberate one-off: prefix the command with CK_ALLOW_DESTRUCTIVE=1 or AGY_ALLOW_DESTRUCTIVE=1`;
  ...
  if (b) {
    console.log(JSON.stringify({ decision: 'deny', reason: b }));
    console.error(b);
    process.exit(2);
  }
