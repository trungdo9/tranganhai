...
if (require.main === module) {
  try {
    ...
    if (data.toolCall && data.toolCall.args) {
      command = data.toolCall.args.CommandLine || data.toolCall.args.command || '';
    ...
    if (violation) {
      const reason = `Blocked directory pattern — '${violation}' traverses a heavy directory (node_modules/__pycache__/.git/dist/build). Excluding it (--exclude-dir, grep -v, -prune) is allowed; reading inside it is not.`;
