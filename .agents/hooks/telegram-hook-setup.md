...
The Telegram hook (`telegram_notify.sh`) automatically sends notifications when Antigravity sessions stop or subagents complete tasks. It provides detailed summaries including tool usage, files modified, and operation counts.
...
#### Option C: `.agents/.env` (Project-Level Override)
...
#### Option D: `.agents/hooks/.env` (Hook-Specific)
...
- `"Stop"`: Triggers when main Antigravity session ends
...
```bash
chmod +x .agents/hooks/telegram_notify.sh
```
...
}' | ./.agents/hooks/telegram_notify.sh
```
...
**Triggered when:** Main Antigravity session ends (user stops Antigravity or task completes)
...
**Cause:** Antigravity hook configuration incorrect or hook script not executable

**Solutions:**

1. **Verify `.agents/config.json` exists and is valid JSON:**
   ```bash
...
2. **Check hook configuration:**
   ```bash
   cat .agents/config.json | jq '.hooks'
   ```

3. **Verify script is executable:**
   ```bash
   ls -l .agents/hooks/telegram_notify.sh
...
**Script Location:** `.agents/hooks/telegram_notify.sh`
