...
description: Execute the 10-phase end-to-end marketing campaign pipeline (Setup -> Research -> Insights -> Strategy -> Plan -> Create -> Edit -> Publish -> Promote -> Measure -> Optimize).
---
...
**Track A:** Copy (blog, social, email, ads). Skills: `copywriting`, `copy-editing`, `content-strategy`. SEO articles at scale → delegate to the `seo-writing` pipeline (`/mk:seo plan` → `write`); SEO-led campaigns may run the full `.agents/workflows/seo-workflow.md` loop instead of Phases 5–10 here.
...
3. **CRO check** — value prop, CTA, friction. Load `.agents/workflows/cro-framework.md` (25-point checklist). Skills: `cro`, `signup`. Owns: CTAs, forms, above-fold copy.
