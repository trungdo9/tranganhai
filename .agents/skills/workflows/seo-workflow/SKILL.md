---
name: seo-workflow
description: Execute the 7-phase closed-loop SEO campaign pipeline (Gate -> Baseline -> Plan -> Write -> Publish -> Distribute -> Measure -> Optimize).
---
...
**Conventions:** every phase writes one artifact into the campaign dir; `pipeline.md` is the single source of truth for article state — phases only pick up rows matching their entry status, so any phase re-runs alone without redoing the rest. Sub-references: `.agents/workflows/marketing-rules.md` (content quality), `.agents/workflows/automation-rules.md` (PII, idempotency, MCP fallback), `skills/marketing/seo-writing/SKILL.md` (stage playbooks).
