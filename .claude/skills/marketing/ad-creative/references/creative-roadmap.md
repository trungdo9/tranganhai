...
|---|---|---|
| **Account performance** | Winners/losers by angle, hook, format; funnel metrics per concept (see [hook-system.md](hook-system.md) diagnostic funnel); fatigue state | platform reporting exports (Google Ads / Meta Ads Manager / LinkedIn / TikTok — CSV or pasted), read per Mode 2 in SKILL.md |
...
| **External organic** | What the niche watches unpaid: top organic content, its hooks, formats, vocabulary; competitor ads running long enough to be presumed working | ad-library and organic-post research (WebSearch/WebFetch), [[social-content]], [[competitor-profiling]] |
...
Maintain one living document (`plans/marketing/<campaign>/ads/creative-roadmap.md`) with three horizons:
...
Last step of the loop, first input of the next one. One artifact per month (`plans/marketing/<campaign>/ads/retros/YYYY-MM.md`):
...
- **Enough data before verdicts** — respect the impression/spend thresholds in Common Mistakes and the kill/keep/scale thresholds in [[ads]]; a two-day read is a coin flip.
...
To run this loop on a schedule (retro on the 1st, weekly refresh Mondays, daily batches via Mode 3), drive it from `/mk:ads creative` on a fixed cadence and keep the artifacts under the campaign folder — ClauKit ships no loop scheduler.
