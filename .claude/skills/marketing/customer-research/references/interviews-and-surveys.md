...
3. **Online sleuthing** — Sales Safari and watering-hole mining (see `source-guides.md` (same folder))
...
Safari is passive (you observe) where interviews are active (you ask). Run it first: it tells you what to ask about, and in whose words. For per-platform search operators and extraction tips, see `source-guides.md` (same folder).
...
- **Mine public sources** (the passive Safari half) via `source-guides.md` (same folder).
...
- Incentive amounts above are the upstream author's figures (US, 2025). Treat them as a starting benchmark, not a rule — adjust for market and role seniority.
- Redact PII before any transcript, quote, or survey verbatim is written into `plans/marketing/` (`.claude/workflows/automation-rules.md`). Quote the language, attribute to a segment, never to a named person.
- Findings go into `plans/marketing/<research>/report.md` with a confidence label and the observation that would disprove them.

Source: `coreyhaines31/marketingskills` (MIT, (c) 2025 Corey Haines), adapted for ClauKit.
