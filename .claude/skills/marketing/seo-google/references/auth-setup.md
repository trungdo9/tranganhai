...
| Type | Used By | Cost |
|------|---------|------|
| **API Key** | PageSpeed Insights, CrUX, CrUX History, Knowledge Graph, YouTube, NLP | Free |
...
3. Name it (e.g., "ClauKit SEO") and note the project ID
...
3. Copy the generated API key and store it securely (env var, not committed to the repo)

## Step 4: Create a Service Account

1. **IAM & Admin > Service Accounts > Create Service Account**
2. Name: `claukit-seo` (or similar)
3. Skip optional permissions steps
4. Click on the created service account > **Keys > Add Key > Create new key > JSON**
5. Download the JSON file and store it securely (e.g., `~/.config/claukit/service_account.json`)
...
Prefer environment variables over a committed config file (see below). If a local config file is more convenient for a single-user setup, keep it outside the repo, e.g. `~/.config/claukit/google-api.json`:

```json
{
  "service_account_path": "~/.config/claukit/service_account.json",
...
Never write API keys, service-account paths, or property IDs into `plans/marketing-context.md` or any other file that gets committed.
...
Confirm each credential works with a minimal call before running a full workflow:

```bash
# API-key tier — PageSpeed Insights
curl -s "https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=https://example.com&key=$GOOGLE_API_KEY" | head -c 200

# Service-account tier — Search Console (requires an OAuth bearer token minted from the service account)
curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
  "https://www.googleapis.com/webmasters/v3/sites" | head -c 200
```

A `200` with JSON back confirms the tier. A `403` means the service account isn't yet added to the property (Step 5/6); a `400` on GSC usually means the property URL format is wrong.

## Environment Variable Alternatives

Instead of (or in addition to) a config file:
