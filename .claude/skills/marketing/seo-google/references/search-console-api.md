...
- `rowLimit` is per API request (max 25,000); page internally with `startRow` if a workflow needs a larger total.
...
- Only present a totals figure as authoritative once it comes from a
  dimensionless aggregate query — never a sum of `query`/`page` rows.
...
There is no native batch endpoint — for a list of URLs, call `index:inspect` once per URL and respect the per-site quota below. Space calls out (or backoff on 429) rather than firing them concurrently.
...
This reports submitted counts only — use URL Inspection above for whether a specific URL is actually indexed.
