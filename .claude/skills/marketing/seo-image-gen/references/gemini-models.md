...
> Do not treat these as Google-confirmed current model IDs — verify against the installed MCP's configuration and https://ai.google.dev/gemini-api/docs first.
...
- **Status:** Base model deprecated March 9, 2026. **Image generation variant may still be accessible**. Use at your own discretion. Prefer 3.1 Flash.
...
Control output resolution via the connected tool's resolution parameter (commonly `imageSize`). Note the **uppercase K** convention used by the Gemini API.

| Resolution Value | Pixel Range | Model Availability | Use Case |
|-------------------|-------------|-------------------|----------|
...
- Default is `1K` if resolution is not specified

## API Configuration (for reference — most MCPs wrap this)
...
Some sources suggest NB2 pricing may run closer to ~$0.067/img than the documented $0.039. Verify current pricing at https://ai.google.dev/gemini-api/docs/pricing before quoting to a user.
...
- No transparent backgrounds (PNG but always with background — see the post-processing green-screen workaround)
...
- Resolution and thinking-level parameters depend on the connected MCP's version/support
