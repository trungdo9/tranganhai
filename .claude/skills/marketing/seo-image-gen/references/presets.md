...
> Load this on-demand when the user asks about presets or brand consistency. The preset manager script/path below (`presets.py`, `~/.banana/presets/`) belongs to the source project's standalone tool — ClauKit doesn't ship it. If no such tool is installed, keep presets as plain JSON alongside the campaign's plan (e.g. `plans/marketing/<campaign>/image-presets/NAME.json`) using the same schema, and load them by hand.
...
## How Presets Merge into the Reasoning Brief

When a preset is active, use its values as defaults for the Reasoning Brief (see `SKILL.md` / `prompt-engineering.md`):
...
If the source project's preset manager is installed:
```bash
# List presets
presets.py list
...
Otherwise, read/write the preset as a plain JSON file matching the schema above wherever the campaign keeps its marketing plan assets.
