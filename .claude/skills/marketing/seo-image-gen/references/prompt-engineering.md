# Prompt Engineering Reference — AI Image Generation for SEO Assets
...
>
> This is imported prompting guidance from the source project, not a Google-published prompting guide — treat specifics as a starting point, not an authoritative spec, unless verified against current Gemini docs.
...
Use a multi-turn chat tool if the connected MCP exposes one, and maintain descriptive anchors:
...
**Multi-image reference technique** (where the model/MCP supports it):
...
- The model preserves features across different angles, actions, and environments
...
For images based on real-world data (weather, events, statistics), a model with search-grounding support can incorporate live information — useful for infographics with current data, where the MCP/model configuration supports it.
...
When adapting prompts from other image-gen syntaxes (Midjourney/DALL-E/etc.) to Gemini's natural-language format:

| Source Syntax | Gemini Equivalent |
|---------------|-------------------|
| `--ar 16:9` | Configure aspect ratio via the connected tool separately |
...
6. **Ignoring aspect ratio** — always set it before generating
...
10. **Not iterating with follow-up prompts** — use multi-turn refinement instead of trying to get everything right in one generation

## Proven Prompt Templates

> These patterns consistently produce high-quality results, per the source project's testing. Use them as starting points and adapt to the request.
...
9. **End with "ultra-realistic, high resolution"** — these two specific anchors help. Avoid generic stacking like "8K, masterpiece, best quality" which adds no value
...
Output-side safety filters cannot be disabled. When a prompt is blocked, the only path forward is rephrasing.
...
Output-side filtering analyzes the generated image, not just the prompt. Even well-phrased prompts can be blocked if the model's interpretation triggers the output filter. When this happens, try shifting the visual concept further from the trigger rather than just changing words.
