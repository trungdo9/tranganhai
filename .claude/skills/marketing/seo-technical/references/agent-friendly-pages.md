...
Web Vitals (see the parent skill's §6), but the agent-UX concern is broader —
it covers page-to-page stability, not just within-page shift.
...
this category** (disabled in Lighthouse 13.4.0); CrUX never provides these lab
audits.
...
If browser automation is available in the environment (e.g. a Playwright MCP
tool), an accessibility-tree snapshot plus a headless render is the natural
way to automate this check; otherwise treat this checklist as manual-review
guidance during a live audit.
