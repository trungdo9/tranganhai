...
This reference is loaded by [[seo-local]] for local-audit work; the root `seo` orchestrator uses it when local-service signals trigger a full-audit dispatch. See also `schema-types.md` (this same directory) for the non-local schema catalog.
