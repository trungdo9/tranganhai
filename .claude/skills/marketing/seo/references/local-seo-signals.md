...
### Industry-specific directories: see `local-schema-types.md` (this same directory)
...
See [[seo-geo]] for how these AI-search citation patterns feed the GEO workflow.
...
This reference is loaded by [[seo-local]] for local-audit work; the root `seo` orchestrator uses it when local-service signals trigger a full-audit dispatch.
