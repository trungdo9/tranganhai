...
The canonical methodology this skill uses to assemble raw sibling-skill findings
into a strategically coherent recommendation set. Every full-site audit and
...
- Raw HTML + rendered HTML (via `WebFetch`, or a browser-automation MCP when JS rendering is required)
- Schema.org markup actually present (via [[seo-schema]])
- SERP visibility for the site's published topics (via [[seo-dataforseo]] / [[seo-google]] when a connection is available)
- Backlink + brand-mention landscape (via [[seo-backlinks]])
- Core Web Vitals field data (via a CrUX/PageSpeed API call, or [[seo-google]])
...
- Assuming a finding from a past audit still applies (Google updates change the
  ground — recheck currency before repeating a stale recommendation)
...
- Read user reviews / community discussions / Reddit threads for what
  ...
  they ask about).
- Read `plans/marketing-context.md` and any prior audit report for this
  target — the user may have already ruled out approaches.
...
- What does **Google's primary-source guidance** say about the
  ...
  to Google (see [[seo-geo]] for the AI-search-specific guidance).
...
Combine findings from sibling skills the user wouldn't naturally pair.
...
- [[seo-schema]] missing Product schema × [[seo-ecommerce]] machine-readable
  buying data missing → both close the same agent-era buying gap; bundle as one
  recommendation.
- [[seo-geo]] low AI-citation rate × [[seo-backlinks]] brand-mention
  underweight → mentions matter far more than raw backlinks for AI
...
**Discipline:** any single sibling-skill finding that survives connection
unchanged should be treated with skepticism — it might be a symptom, not a cause.
...
- Which recommendations need a **data source that's not yet available**
  (e.g. a DataForSEO connection for SERP data, a GSC/GA4 connection for
  traffic)? Flag the gap rather than silently skipping it.
...
- A markdown report with prioritized actions, dependencies, and
  measurable outcomes, written to `plans/marketing/<target>/seo-audit-report.md`.
- Generated schema JSON-LD ready to paste into the site (via [[seo-schema]]).
- A content brief with target keywords, outline, and internal links (via [[seo-content-brief]]).
...
- Capture a baseline via [[seo-drift]] so subsequent audits can prove what changed.
- Define one or two leading indicators the user should monitor (CWV
...
- Schedule a re-audit cadence appropriate to the site's velocity
  (weekly for a high-churn e-commerce site; quarterly for a B2B SaaS).
- Surface what this audit **could not measure** (offline
...
Every full-site audit and deep-page audit walks through PERCEIVE → ANALYZE →
VALIDATE → ACT before emitting the action plan. The Critical / High / Medium /
Low priority bucketing happens **after** the validation phase, not instead of it.

Single-purpose requests (schema-only, images-only, technical-only, etc.) can
skip the full loop when the user is asking a narrow question — but their
recommendations should still pass at least THINK + ACCEPT before being
emitted (does this rest on a sound first principle, and is the falsifiability
surfaced?).

## When to escalate to the user

These principles belong to this skill; they are not the user's. Surface them
for the user when:
