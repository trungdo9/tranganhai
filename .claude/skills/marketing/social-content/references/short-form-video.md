...
Hooks, scripts, and production patterns for organic short-form video on TikTok, Instagram Reels, and YouTube Shorts.

Boundary: this is **organic** short-form. Paid video creative (UGC ads, faceless motion ads, platform ad specs) is [[ad-creative]] and its `references/short-form-video-specs.md`. Full video production — voiceover, AI visuals, render — is the `/mk:video` pipeline and the `video-producer` agent; this file owns the script and the platform-native cut.

Posting times, optimal lengths, and platform behaviour below are the upstream source's observations. Verify against current platform guidance and the account's own analytics before treating any of them as fixed.
...
- AI voices work for faceless content; the `/mk:video` pipeline owns TTS generation
