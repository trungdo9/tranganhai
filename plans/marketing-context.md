...
```mermaid
graph LR
    subgraph TrangAnhPalette["BẢNG MÀU THƯƠNG HIỆU TRANG ANH AI"]
        C1["<b>Deep Slate (#1E293B)</b><br/>60% Nền tảng & Sự êm mắt<br/>Vững chãi, Chuẩn mực"]
        C2["<b>Brand AI Blue (#4F46E5)</b><br/>30% Trí Tuệ & Công Nghệ<br/>Mềm mại, Hiện đại, Tinh tế"]
        C3["<b>Sage Teal (#0D9488)</b><br/>10% Điểm Nhấn & Tăng Trưởng<br/>An tâm, Tươi mới, Call-To-Action"]
    end
```
...
```mermaid
graph TD
    subgraph StrategicShift["CHUYỂN DỊCH ĐỊNH VỊ CỐT LÕI CỦA TRANG ANH AI"]
        LegacySEO["<b>BẪY DỊCH VỤ RỜI RẠC & SEO TRUYỀN THỐNG</b><br/>• Bán công cụ AI lẻ tẻ (AI Wrapper) dễ bị đào thải<br/>• Bị ép cam kết vị trí Top Google rủi ro ngoại cảnh<br/>• Dữ liệu công ty nằm rải rác Zalo, Excel, không ai quản<br/>• Tốn nhiều bài viết rác nhưng không tạo ra vận hành thực"]
        
        AIAgentOps["<b>AI AGENT OPERATIONS & WORKFLOW ALIGNMENT (TRANG ANH AI)</b><br/>• 1. Master RAG Lake (Zero Hallucination): Số hóa & cấu trúc hóa tri thức<br/>• 2. Báo cáo Quản trị 1 Trang (Executive BI) & Cố vấn Chiến lược CEO<br/>• 3. Bộ đôi thực chiến: Zalo Copilot 8s (Human-in-the-loop) + CSKH 24/7<br/>• 4. Căn chỉnh (Align) thông suốt luồng: Data -> Content -> Sales -> Báo cáo"]
        
        LegacySEO -.->|Loại bỏ rủi ro| AIAgentOps
    end
```
...
```mermaid
graph TD
    subgraph G1["NHÓM A: NỖI ĐAU VẬN HÀNH & KINH DOANH"]
        A1["Tuyển và giữ nhân sự khó (lương 12-16tr/tháng)"]
        A2["Phản hồi báo giá chậm 15-30 phút làm mất đơn vào tay đối thủ"]
        A3["Dữ liệu rải rác: Zalo, Excel, Drive, catalogue cũ nát"]
        A4["Thuê ngoài nhiều bên lắt nhắt: Output chắp vá, lệch Brand Voice"]
        A5["Ban Giám Đốc thiếu báo cáo số liệu tức thì để ra quyết định"]
    end
    
    subgraph G2["NHÓM B: BỘI THỰC CÔNG CỤ & KHÁNG CỰ NỘI BỘ"]
        B1["Mua nhiều tool SaaS rời rạc nhưng nhân viên không dùng"]
        B2["Bẫy Copy-Paste thủ công giữa Zalo, Excel và phần mềm"]
        B3["Nhân sự lo sợ bị AI thay thế nên kháng cự ngầm"]
        B4["Ngợp thuật ngữ, không biết bắt đầu ứng dụng AI từ khâu nào"]
    end

    subgraph G3["NHÓM C: RỦI RO PHÁP LÝ & AN TOÀN DỮ LIỆU"]
        C1["Dữ liệu nội bộ bị dán tràn lan lên AI công cộng"]
        C2["Nguy cơ phạt tới 5% doanh thu theo Luật Dữ Liệu 91/2025/QH15"]
        C3["Chưa gán nhãn máy đọc & thiếu Human-in-the-loop theo Luật AI 134/2025"]
    end
```
...
sequenceDiagram
    autonumber
    actor Khach as Khách Hàng (Zalo)
    actor Sales as Nhân Viên Sales
    participant Copilot as Zalo Copilot (Trang Anh AI)
    participant RAG as Master RAG Lake (SQL Pricing)
    participant PDF as Vector PDF Engine (Typst)

    Khach->>Sales: Nhắn hỏi giá (Kèm tiếng lóng, ảnh chụp tem nhãn, danh sách viết tắt)
    Note over Sales,Copilot: BƯỚC 1: Sales nhấn 1-chạm copy tin nhắn/ảnh dán vào Copilot (2s)
    Sales->>Copilot: Input văn bản / hình ảnh
    Note over Copilot,RAG: BƯỚC 2: Copilot bóc tách tiếng lóng, Fuzzy Matching mã hàng (2s)
    Copilot->>RAG: Truy vấn bảng giá xác định (Deterministic SQL)
    RAG-->>Copilot: Trả về đơn giá, tồn kho, quy cách chuẩn
    Note over Copilot,PDF: BƯỚC 3: Render báo giá PDF Vector chuẩn Print + VietQR động (2s)
    Copilot->>PDF: Generate Vector PDF (Typst Engine)
    PDF-->>Sales: Trả về file PDF hoàn chỉnh & tin nhắn tóm tắt báo giá
    Note over Sales,Khach: BƯỚC 4: Sales liếc mắt kiểm tra 2s -> Bấm gửi Zalo cho khách (2s)
    Sales->>Khach: Gửi file Báo giá PDF chuyên nghiệp + Mã VietQR thanh toán
```
...
```mermaid
graph LR
    subgraph RevenueEngine["CƠ CẤU DOANH THU RETAINER & GÓI CHÀO HÀNG ĐỈNH CAO"]
        Setup["<b>PHÍ SETUP & MASTER RAG LAKE (NIÊM YẾT)</b><br/>20.000.000 - 35.000.000 đ<br/><i>(Làm sạch data, Vector Lake, RAG Engine, Đào tạo 4 tuần)</i>"]
        Offer["<b>THE ULTIMATE 0-SETUP OFFER</b><br/><b>TẶNG MIỄN PHÍ 100% SETUP</b><br/><i>Khi cam kết Hợp đồng Retainer 06 tháng (Thu trước Quý 1)</i>"]
        Retainer["<b>RETAINER DUY TRÌ & TỐI ƯU HÓA HÀNG THÁNG</b><br/>• Gói 1: Nền tảng — <b>12.500.000 đ/tháng</b><br/>• Gói 2: Tăng trưởng — <b>24.000.000 đ/tháng</b> (Chủ lực)<br/>• Gói 3: Toàn diện — <b>41.500.000 đ/tháng</b>"]
        
        Setup ==>|Ký HĐ 6 tháng| Offer ==>|Duy trì vận hành| Retainer
    end
```
...
```mermaid
graph TD
    subgraph MoatFramework["HỆ THỐNG 4 HÀO KINH TẾ PHÒNG THỦ CỦA TRANG ANH AI"]
        M1["<b>HÀO 1: DOMAIN DATA & TECHNICAL SPECIFICATION MOAT</b><br/>• Tri thức kỹ thuật B2B sâu sắc (Iodine, RO, xử lý khí thải, QCVN/ASTM)<br/>• LLM phổ thông không có Master Lake sẽ bị ảo giác 100%"]
        M2["<b>HÀO 2: HIGH SWITCHING COST (CHI PHÍ CHUYỂN ĐỔI CAO)</b><br/>• Toàn bộ bảng giá, quy tắc chiết khấu, luồng Zalo Copilot đã nhúng vào vận hành<br/>• Rời bỏ AI đồng nghĩa tuyển thêm 2-3 nhân sự với chi phí gấp đôi"]
        M3["<b>HÀO 3: DEEP SOP EMBEDMENT (NHÚNG SÂU VĂN HÓA VẬN HÀNH)</b><br/>• Đào tạo 3 tầng nhân sự tạo thói quen duyệt việc 10 giây<br/>• Nhân sự tạo sự kháng cự nếu lãnh đạo muốn quay lại làm thủ công"]
        M4["<b>HÀO 4: COMPLIANCE & TRUST MOAT (LÁ CHẮN PHÁP LÝ)</b><br/>• Tài khoản chính chủ tuân thủ Luật 91/2025 & Luật AI 134/2025<br/>• Hồ sơ minh bạch giúp khách hàng dễ dàng làm việc với đối tác FDI"]
...
```mermaid
gantt
    title LỘ TRÌNH 4 TUẦN DONE-WITH-YOU VẬN HÀNH THỰC CHIẾN
    dateFormat  YYYY-MM-DD
    section Tuần 1: Data & Master Lake
    Khảo sát dữ liệu & Tài khoản chính chủ      :done,    des1, 2026-09-01, 3d
    OCR, Unmerge Excel & Xây Master RAG Lake    :active,  des2, 2026-09-03, 4d
    section Tuần 2: Setup Multi-Agent
    Cài đặt Agent Engine & Brand Voice           :         des3, 2026-09-08, 4d
    Thiết lập Zalo Copilot & Chatbot 24/7       :         des4, 2026-09-11, 3d
    section Tuần 3: Workflow Alignment
    Test bóc tách báo giá 8s & Chốt luồng CRM    :         des5, 2026-09-15, 4d
    Cấu hình Executive BI Dashboard CEO          :         des6, 2026-09-18, 3d
    section Tuần 4: Đào tạo & Chuyển giao
    Đào tạo 3 Tầng Nhân Sự & Bàn giao SOP        :         des7, 2026-09-22, 5d
    Ký nghiệm thu & Chuyển sang Retainer         :         des8, 2026-09-26, 2d
```
