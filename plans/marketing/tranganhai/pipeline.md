...
1. **Publish target.** The live site (`tranganhai.vercel.app` — updated 2026-09-09 from the prior preview domain `web-gamma-livid-65.vercel.app`) is Next.js/Vercel, not WordPress — the `wordpress-rest` Stage-6 path doesn't apply. How will articles actually get published (a CMS behind this Next.js site — e.g. headless CMS/MDX/git-based content — or is `tranganhai.com`, the domain named in the context hub, the real intended publish target instead)? Need this answered before any `write --publish` run.
